Blocks
===========================================
**Important Note!** In Omnia 7.0 and later blocks that are tied to a certain feature in Omnia are only available if that feature is activated. This will be noted in the documentation of each block but it will take some time before all block descriptions are updated.

.. toctree::
   :titlesonly:

   
   accordion/index
   button-link/index
   activity-feed/index
   announcements/index
   banner/index
   blocks-event-management/index
   blocks-forms/index
   breadcrumb/index
   calendar-rollup/index
   channel-rollup/index
   create-app/index
   create-teamwork/index
   community-rollup/index
   current-navigation/index
   document-rollup/index
   faq/index
   general-block-settings/index
   governance-dashboard/index
   html-script/index
   iframe/index
   media/index
   mega-menu/index
   metrics-block-612/index
   my-subscriptions/index
   notification-panel/index
   omnia-block-webpart/index
   organization-tree/index
   page-feedback/index
   page-likes-and-comments/index
   page-properties/index
   page-rollup/index
   app-posts/index
   variation-selector/index
   people-rollup/index
   power-app/index
   process-blocks/index
   publishing-app-rollup/index
   user-profile-completion/index
   qr-code/index
   quick-links/index
   rating/index
   related-links/index
   rss-reader/index
   search/index
   sign-off-requests-rollup-613/index
   subscribe/index
   task-rollup/index
   taxonomy-navigation/index
   team-collaboration-rollup/index
   team-news-rollup/index
   teams-channel-feed/index
   text/index
   working-with-blocks/index
   yammer-feed/index
   
